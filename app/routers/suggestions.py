...
async def get_suggestions(request: Request, resume_file: UploadFile = File(...)):
    try:
        ...
        cached_suggestions = await suggestion_cache.get(session_id,resume_text[:20].encode())
        ...
        # Save in Redis
        await suggestion_cache.save(session_id,resume_text[:20].encode(), suggestions)
